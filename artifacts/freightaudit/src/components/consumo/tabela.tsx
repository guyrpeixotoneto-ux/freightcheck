 ...
 *
 * **A coluna "Unidade" decide um mal-entendido caro nesta tabela.** `2,50` e
 * `2,40` em duas linhas seguidas do mesmo trecho são um rendimento e um custo — e
 * sobem em sentidos opostos. Sem a unidade ao lado do nome, as duas se leem como
 * "subiu 0,1", e uma delas quer dizer o contrário disso.
 *
 * É a mesma razão de a cor da diferença ser decidida pelo **papel** da coluna, em
 * `corDaDiferenca`: um km/l que sobe é verde, e um R$/km que sobe é vermelho.
 *
 * **O selo de estado tem texto, e não só cor.** Quem não distingue o verde do
 * vermelho continua lendo "Alterado" e "Conflito".
 */
export function TabelaDeConsumo({
  linhas,
  justificadaPor,
  onAbrir,
  onJustificar,
  ...
  onAbrir: (linha: LinhaDeConsumo) => void;
    ...
    <div className="superficie overflow-x-auto">
      <table className="w-full min-w-[58rem] border-collapse text-sm">
        <caption className="sr-only">
          Comparação do consumo entre a vigência base e a comparada, por trecho e variável.
        </caption>
        <thead>
