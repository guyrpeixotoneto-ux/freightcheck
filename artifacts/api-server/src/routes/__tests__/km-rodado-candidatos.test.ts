...
/**
 * `GET /km-rodado/candidatos` — o que cada candidata a "De" produz contra o "Para".
 *
 ...
 *
 * O grão é **trecho**, e não veículo — e é a única diferença para as quatro
 * rubricas de custo fixo.
 */
...
beforeAll(async () => {
  ctx = await createTestDatabase("api_km_rodado_candidatos");
  ...
  const { default: kmRodadoRouter } = await import("../km-rodado");
  const app = express();
...
describe("GET /km-rodado/candidatos", () => {
  it("exige a vigência de destino", async () => {
    const res = await get("/km-rodado/candidatos");
  ...
  it("só oferece candidatas da mesma unidade e da mesma cobertura", async () => {
    ...
    const { status, body } = await get(`/km-rodado/candidatos?para=${destino.id}`);
   ...
   * menu tem de bater com `/km-rodado/comparacao` para cada um dos dois pares. Uma
  ...
  it("o número do menu concorda com a comparação dos dois pares", async () => {
    ...
    const primeira = await get(`/km-rodado/candidatos?para=${destino.id}`);
    const segunda = await get(`/km-rodado/candidatos?para=${outroDestino!.id}`);
    ...
    for (const [resposta, para] of [
      ...
      const { body: comparacao } = await get(
        `/km-rodado/comparacao?base=${comum}&comparada=${para}`,
      );
   ...
   * ao lado da vigência bate, campo a campo, com o que `/km-rodado/comparacao`
   * responde para aquele par exato. Se as duas divergirem, quem escolhe pelo
   * menu escolhe por um número que a tela não confirma.
   */
  it("o número do menu é o mesmo de /km-rodado/comparacao para aquele par", async () => {
    const lista = await vigencias();
    const destino = lista[0];
    const { body } = await get(`/km-rodado/candidatos?para=${destino.id}`);
    ...
    const { body: comparacao } = await get(
      `/km-rodado/comparacao?base=${comNumero.id}&comparada=${destino.id}`,
    );
  ...
  it("ausência de cálculo é null, e nunca um zero inventado", async () => {
    const lista = await vigencias();
    const { body } = await get(`/km-rodado/candidatos?para=${lista[0].id}`);
  ...
  it("recusa a vigência de outra operação", async () => {
    const lista = await vigencias();
    const res = await get(`/km-rodado/candidatos?para=${lista[0].id}&operacao=ROTA`);
  ...
  it("a conexão já voltou ao pool quando a resposta chega", async () => {
    const lista = await vigencias();
    const { status } = await get(`/km-rodado/candidatos?para=${lista[0].id}`);
