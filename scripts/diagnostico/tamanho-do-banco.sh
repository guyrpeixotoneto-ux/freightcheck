#!/usr/bin/env bash
# ===========================================================================
# DIAGNÓSTICO DE TAMANHO DO BANCO — SOMENTE LEITURA, SOMENTE PRODUCTION
# ===========================================================================
# Executa scripts/diagnostico/tamanho-do-banco.sql contra Production. Nenhuma
# escrita: não migra, não apaga, não compacta, não reorganiza, não roda VACUUM.
#
...
#
# Uso:  PRODUCTION_DATABASE_URL='postgres://…' ./scripts/diagnostico/tamanho-do-banco.sh
# ===========================================================================
set -Eeuo pipefail

AQUI="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SQL="${AQUI}/tamanho-do-banco.sql"
...
[ -r "$SQL" ] || erro "tamanho-do-banco.sql não encontrado em ${AQUI}."
...
# catálogo são negadas por permissão (`pg_ls_waldir()` é a certa), e parar no
# primeiro "permission denied" devolveria um diagnóstico pela metade. Cada seção
# do .sql tem transação própria, então a recusa de uma não contamina as outras;
# o erro aparece no lugar dela, nomeado, e o resto continua.
