...
import { listarCategorias, listarSignificados } from "../catalogo";
 ...
 * a planilha grava confirma atributo nenhum.** Nome, descrição, fórmula,
 * significado e categoria entram; `semantics_status`, `confirmed_by` e
...
async function catalogos() {
  const [significados, categorias] = await Promise.all([
    listarSignificados(ctx.db),
    listarCategorias(ctx.db),
  ]);
  return {
...
describe("a planilha de atributos descreve, e só", () => {
  it("grava prosa, significado e categoria sem tocar no status", async () => {
      ...
      [
        {
          ...
          significado: lista.significados[0].label,
  ...
  it("escreve o significado e a categoria como proposta, na versão em vigor", async () => {
    const depois = await atributo(CODE);
    const lista = await catalogos();

    const [significado] = await ctx.db
      .select()
      .from(semanticMeaningTable)
      .where(eq(semanticMeaningTable.code, lista.significados[0].code));
    const [no] = await ctx.db
      .select()
      .from(taxonomyNodeTable)
      .where(eq(taxonomyNodeTable.code, lista.categorias[0].code));

    expect(depois.meaningId).toBe(significado.id);
    expect(depois.taxonomyNodeId).toBe(no.id);

    // Espelhado na versão em vigor: sem isto, a próxima projeção devolveria o
    // significado antigo e a planilha pareceria não ter sido aplicada.
    const versao = await versaoEmVigor(depois.id);
    expect(versao.meaningId).toBe(significado.id);
  ...
  it("registra no histórico quem trouxe a planilha para dentro", async () => {
    ...
    expect(daPlanilha.length).toBeGreaterThanOrEqual(4);
    expect(daPlanilha.map((e) => e.field)).toEqual(
      expect.arrayContaining(["display_name", "definition", "meaning_id", "taxonomy_node_id"]),
    );
  ...
  it("não grava nada quando a planilha volta igual ao que já está lá", async () => {
    const base = await daBase();
    const lista = await catalogos();
    const linhas = montarLinhas(base.filter((a) => a.code === CODE), lista);

    const conferencia = conferirPreenchimento(
      linhas.map((l) => ({ aba: "Cavalo", linha: 2, ...l })),
      base,
      lista,
    );
  ...
  it("não cria atributo para um código que a base não tem", async () => {
    const base = await daBase();
    const lista = await catalogos();
    const conferencia = conferirPreenchimento(
      [
