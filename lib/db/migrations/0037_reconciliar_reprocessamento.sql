-- Reconciliar o que a 0036 cria e o bridge remove.
--
...
--
-- A `0036` acrescentou duas colunas, um índice e duas constraints a
...
--
-- Uma migration nova, e não uma edição da `0034`, pelo motivo de sempre nesta
-- fila: a `0034` já está registrada em todo banco que a aplicou, e reescrevê-la
...
-- mesmo arquivo, que é a confusão que a `0036` desfaz.
DO $reentrante$
BEGIN
  IF NOT EXISTS (
...
-- Quem aponta, explica. A recíproca não vale de propósito — ver a `0036`.
DO $reentrante$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint c
